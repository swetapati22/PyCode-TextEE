...
def manage_dataset(dataset, key, shots = None, shot_data = None):
    ...
    with open(f"/scratch/spati/tmp/LLaMA/datasets/richere_test100_guidelinePN_Adv_w_NS_iter1_{key}_sole_data.json", "w") as f:
        json.dump(dataset_dict, f, indent = 4)
    # Convert to Dataset format
    return Dataset.from_dict(dataset_dict) 

def richere_test100_guidelinePN_Adv_w_NS_iter1(shot_data = False):
    ...
    # )
    train_data, valid_data = json.load(open("/scratch/spati/tmp/NLP_Research_Work/TextEE/data/guideline_experiments_richere/richere_15random_adv_guidelines/final_data_split/PA_iter1_100_train_w_neg.json")), json.load(open("/scratch/spati/tmp/NLP_Research_Work/TextEE/data/guideline_experiments_richere/richere_15random_adv_guidelines/final_data_split/test_final.json"))
