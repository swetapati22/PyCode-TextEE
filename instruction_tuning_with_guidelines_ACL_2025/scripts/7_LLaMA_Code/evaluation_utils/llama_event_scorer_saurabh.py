...
def micro_e2e_scores(json_data):
    ...
    6- PrettyTable : DONE
    """
    print("json_data in micro e2e score",len(json_data))
    trigger_id_pred_num, trigger_id_gold_num, trigger_id_match_num = 0, 0, 0
    trigger_cls_match_num = 0 # pred num, gold_num for both id and cls are same so only above 2 vars can be reused
    arg_id_pred_num, arg_id_gold_num, arg_id_match_num = 0, 0, 0
    arg_cls_match_num = 0 # pred num, gold_num for both id and cls are same so only above 2 vars can be reused
...
def evaluator(json_data, original_json_data):
    ...
    print("original_json_data(raw_data) in evaluator: ", len(original_json_data))
    print("json_data(checkpoint) in evaluator: ", len(json_data))
    ...
    for dataset_split in datasets_splits:
        ...
        print(f"Dataset type: {dataset_split}, no: {len(datasets_splits[dataset_split])}")
        # print(datasets_splits[dataset_split])
        print(f"Calling function {function} for the task type: {dataset_split}")
        print("lenth of dataset from the evaluator: ", len(datasets_splits[dataset_split]))
        print("lenth of checkpoint from the evaluator: ", len(json_data))
...
# if __name__ == "__main__":
    # # add = "/scratch/spati/tmp/LLaMA/checkpoints/inst_event_llama_rq6_with_X_Y_split_rank_128_alpha_64_lr_1e-05"
    # add = "/scratch/spati/tmp/LLaMA/checkpoints/inference_dev4_baseline_infered_rq3_val_numbers_epoch_final.json"
    # # micro_e2e_scores(json.load(open(add)))
    # # xxx
    # directory = os.listdir(add)
    # for jsn_add in directory:
    #     if os.path.isdir(os.path.join(add, jsn_add)) or jsn_add.find("numbers")>=0:
    #         continue
    #     print("-"*100)
    #     print(f"Evaluating file: {jsn_add}")
    #     jsn_add = os.path.join(add, jsn_add)
    #     jsn = json.load(open(jsn_add))
    #     # print("All datasets e2e")
    #     # full = micro_e2e_scores(jsn)
    #     # orig_jsn = json.load(open("/scratch/spati/tmp/NLP_Research_Work/TextEE/data/modified_baseline/maven/dev.json"))#["conversations"]
    #     orig_jsn = json.load(open("/scratch/spati/tmp/NLP_Research_Work/TextEE/data/modified_baseline/inference_3_datasets/ace05-en/dev_100_samples_final.json"))#["conversations"]
...
if __name__ == "__main__":
    # add = "/scratch/spati/tmp/LLaMA/checkpoints/inference_dev1_baseline_eval_results_epoch_final.json"
    # add = "/scratch/spati/tmp/LLaMA/checkpoints/inference_dev4_baseline_infered_rq3_100_eval_results_epoch_final.json"
    add = "/scratch/spati/tmp/LLaMA/checkpoints/inference_dev1_baseline_wo_guidelines_eval_results_epoch_final.json"
    is_directory = os.path.isdir(add)
    ...
    if is_directory:
        ...
        for jsn_add in directory:
            ...
            jsn_add = os.path.join(add, jsn_add)
            jsn = json.load(open(jsn_add))
            # orig_jsn = json.load(open("/scratch/spati/tmp/NLP_Research_Work/TextEE/data/modified_baseline/inference_3_datasets/wikievents/dev_100_samples_final_3set_gt_at_bottom.json"))
            orig_jsn = json.load(open("/scratch/spati/tmp/NLP_Research_Work/TextEE/data/modified_baseline/inference_3_datasets/ace05-en/dev_100_samples_final.json"))
            # orig_jsn = json.load(open("/scratch/spati/tmp/LLaMA/datasets/inference_dev1_baseline_valid_data.json"))
            # orig_jsn = json.load(open("/scratch/spati/tmp/NLP_Research_Work/TextEE/data/modified_baseline/inference_3_datasets/wikievents/dev_100_samples_final_3set.json"))
            # orig_jsn = json.load(open("/scratch/spati/tmp/NLP_Research_Work/TextEE/data/modified_baseline/inference_3_datasets/maven/dev_100_samples_final_3set.json"))


            evaluator(jsn, orig_jsn)
            print("-" * 100)
    else:
        print(f"Evaluating file: {add}")
        jsn = json.load(open(add))
        orig_jsn = json.load(open("/scratch/spati/tmp/NLP_Research_Work/TextEE/data/modified_baseline/inference_3_datasets/wikievents/dev_100_samples_final_3set_gt_at_bottom.json"))
        # orig_jsn = json.load(open("/scratch/spati/tmp/NLP_Research_Work/TextEE/data/modified_baseline/inference_3_datasets/ace05-en/dev_100_samples_final.json"))
        # orig_jsn = json.load(open("/scratch/spati/tmp/LLaMA/datasets/inference_dev1_baseline_valid_data.json"))
        # orig_jsn = json.load(open("/scratch/spati/tmp/NLP_Research_Work/TextEE/data/modified_baseline/inference_3_datasets/wikievents/dev_100_samples_final_3set.json"))
        # orig_jsn = json.load(open("/scratch/spati/tmp/NLP_Research_Work/TextEE/data/modified_baseline/inference_3_datasets/maven/dev_100_samples_final_3set.json"))
        print("orig_jsn(raw_data) from the main function", len(orig_jsn))
        print("jsn(checkpoint) from the main function", len(jsn))
        evaluator(jsn, orig_jsn)
        print("-" * 100)
