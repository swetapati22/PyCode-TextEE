...
def parse_args():
    ...
    parser.add_argument("--dataset_name", type=str, default="guideline_PS_Adv", help="Dataset name for training.")
...
def main(args):
    ...
    if args.do_resume_train:
        base_experiment_dir = "./checkpoints"
        if not os.path.exists(base_experiment_dir):
            print(f"The base directory {base_experiment_dir} does not exist. Starting training from scratch.")
        else:
            experiments = [
                exp for exp in os.listdir(base_experiment_dir)
                if os.path.isdir(os.path.join(base_experiment_dir, exp))
            ]
            if not experiments:
                print("No experiments found in the checkpoints directory.")
            else:
                print("Available experiments:")
                for i, experiment in enumerate(experiments, start=1):
                    print(f"{i}: {experiment}")
                try:
                    experiment_choice = int(input("Enter the number corresponding to the experiment (or 0 to start from scratch): "))
                    if experiment_choice == 0:
                        print("Starting training from scratch.")
                    elif 1 <= experiment_choice <= len(experiments):
                        selected_experiment = experiments[experiment_choice - 1]
                        experiment_path = os.path.join(base_experiment_dir, selected_experiment)
                        
                        checkpoints = [
                            ckpt for ckpt in os.listdir(experiment_path)
                            if os.path.isdir(os.path.join(experiment_path, ckpt)) and ckpt.startswith("checkpoint-")
                        ]
                        checkpoints.sort(key=lambda x: int(x.split("-")[1]))  # Sort by step number
                        if not checkpoints:
                            print(f"No checkpoints found for experiment {selected_experiment}. Starting training from scratch.")
                        else:
                            ...
                            trainer.args.output_dir = experiment_path  # Set output_dir to experiment path
                            trainer.train(resume_from_checkpoint=selected_checkpoint)
                    else:
                        print("Invalid experiment selection. Please try again.")
                except ValueError:
